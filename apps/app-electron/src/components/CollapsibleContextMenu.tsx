...
export default function CollapsibleContextMenu() {
  ...
  // Load context menu width from SQLite when session changes
  useEffect(() => {
  ...
  // Save context menu width to SQLite when it changes (debounced)
  useEffect(() => {
    ...
    saveWidthTimeoutRef.current = setTimeout(async () => {
      try {
        await window.electronAPI.saveAppUIPreference(
          activeSession.id,
          'context_menu_width',
  ...
  }, [contextMenuWidth, activeSession, isLoadingWidth])
